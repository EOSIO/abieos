...
namespace eosio {
   ...
   struct strict_conversion { using reverse = strict_conversion; };
   // Can discard some fields
   struct narrowing_conversion { using reverse = widening_conversion; };
   // Can default construct some fields
   struct widening_conversion { using reverse = narrowing_conversion; };
   ...
   using conversion_kind_t = std::conditional_t<std::is_same_v<decltype(conversion_kind(std::declval<T>(), std::declval<U>())), no_conversion>,
   ...
   auto convert_impl(Field field, const T& src, U& dst, F&& f, int) -> std::void_t<decltype(field(&src)), decltype(field(&dst))> {
